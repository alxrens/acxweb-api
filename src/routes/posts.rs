...
use actix_web::{delete, get, patch, post, web, HttpResponse, HttpRequest};
...
#[post("/posts")]
async fn create(data: web::Data<AppState>, post_from: web::Json<NewPost>) -> Result<HttpResponse, CustomError> {
    let conn = &data.conn;
    let form = post_from.into_inner();
